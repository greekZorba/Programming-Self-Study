...
public class TreeMaximum {
    ...
    public static void main(String[] args){
        ...
        searchTreeMaximum(0, binaryTree.length);


    }

    private static void searchTreeMaximum(int startIndex, int endIndex){
        /** 이진검색트리는 크기에 중간 값이 부모 노드가 되기 때문에 아래와 같은 방식으로 부모 노드를 구현*/
        int parentNodeIndex = (startIndex + endIndex)/2;
        if(parentNodeIndex == endIndex-1){
            System.out.println("트리 중 가장 큰 수 : "+binaryTree[parentNodeIndex]);
            return;
        }

        System.out.println(binaryTree[parentNodeIndex]);
        searchTreeMaximum(parentNodeIndex, endIndex);
